...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const { data: behavioralIncidents, error: incidentsError } = await supabase
    ...
    // High-risk cadets
    const { data: highRiskCadets, error: highRiskError } = await supabase
      .from('cadets')
      .select('*')
      .lte('behavior_score', 2);

    if (highRiskError) throw highRiskError;

    // Average behavior score
    const { data: averageBehaviorScoreData, error: averageBehaviorScoreError } = await supabase
      .from('cadets')
      .select('behavior_score');

    if (averageBehaviorScoreError) throw averageBehaviorScoreError;

    const averageBehaviorScore = averageBehaviorScoreData.reduce((sum, cadet) => sum + cadet.behavior_score, 0) / averageBehaviorScoreData.length;

    // Behavior score distribution
    const { data: behaviorDistribution, error: behaviorDistributionError } = await supabase
      .from('cadets')
      .select('behavior_score');

    if (behaviorDistributionError) throw behaviorDistributionError;

    // HiSET distribution
    const { data: hisetDistribution, error: hisetDistributionError } = await supabase
      .from('cadets')
      .select('hiset_status');

    if (hisetDistributionError) throw hisetDistributionError;

    // Completion rate
    const { data: completionRateData, error: completionRateError } = await supabase
      .from('cadets')
      .select('hiset_status');

    if (completionRateError) throw completionRateError;

    const totalCadets = completionRateData.length;
    const completedCadets = completionRateData.filter(cadet => cadet.hiset_status === 'completed').length;
    const completionRate = totalCadets > 0 ? (completedCadets / totalCadets) * 100 : 0;

    // Average completion time
    const avgCompletionTime = 180;
...
// Event metrics
    ...
    const { data: monthlyEvents, error: eventsError } = await supabase
      ...
      .select(`
        *,
        ...
        event_cadet_assignments(cadet_id)
      `)
    ...
    monthlyEvents.forEach(event => {
      ...
      totalServiceHours += event.community_service_hours || 0;

      if (event.status === 'completed') {
        const assignedCount = (event.event_staff_assignments?.length || 0) + (event.event_cadet_assignments?.length || 0);
        if (assignedCount > 0) {
          totalAttendance += 100; // Assuming 100% attendance for completed events
    ...
    const processedUpcomingEvents = upcomingEvents.map(event => ({
      ...event,
      assigned_staff_count: event.event_staff_assignments?.length || 0
    }));
    ...
    monthlyEvents.forEach(event => {
      event.event_staff_assignments?.forEach(assignment => {
        staffWithEvents.add(assignment.staff_id);
      });
    });
    ...
    const staffParticipationRate = totalStaff?.length > 0 
      ? Math.round((staffWithEvents.size / totalStaff.length) * 100)
      : 0;
    ...
    res.json({
      ...
      totalServiceHours,
      averageAttendance: eventsWithAttendance > 0 ? Math.round(totalAttendance / eventsWithAttendance) : 0,
      staffParticipationRate,
      upcomingEvents: processedUpcomingEvents,
      upcomingEventsCount: processedUpcomingEvents.length
    });
